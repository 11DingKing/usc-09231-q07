...
class ArchiveImportTask(SentinelImportTask):
    ...
    All format-specific handling (recognition, member iteration and
    extraction) lives in the `archives` module; this class only manages
    the temporary directory's lifecycle.
    """
    ...
    def __init__(self, toppath: util.PathBytes) -> None:
        ...
        # Owns the temp extraction directory's whole lifecycle.
        self.extraction = archives.ExtractedArchive(util.syspath(toppath))

    @property
    def extracted(self) -> bool:
        """Whether an extraction directory currently exists."""
        return self.extraction.is_extracted()

    @classmethod
    def is_archive(cls, path: util.StrPath) -> bool:
        """Returns true if the given path points to an archive that can
        be handled.
        """
        return archives.is_archive(path)
    ...
    def cleanup(
        ...
        In ``move`` mode, if the extraction directory contains no files
        after the pipeline has run (i.e. every file in the archive was
        successfully imported) also remove the source archive. Archives
        are preserved on partial imports and in non-move modes.

        Idempotent and safe after a failed extraction: with no live
        extraction directory this is a no-op.
        """
        if not self.extracted:
            return

        all_files_imported = move and not self.extraction.has_remaining_files()
    ...
    def extract(self) -> None:
        ...
        Sets `toppath` to that directory. Every format goes through the
        same open-extract-restore-close sequence in the `archives`
        module; if any step fails midway (corrupt or empty archive,
        missing optional dependency, extraction error), the temporary
        directory is removed before the exception propagates, so failed
        imports never leave files behind.
        """
        extract_to = self.extraction.create()
...
class ImportTaskFactory:
    ...
    def unarchive(self) -> ArchiveImportTask | None:
        ...
        except archives.ArchiveDependencyError as exc:
            # Clear, actionable message naming the missing optional package.
            log.error("{}", exc)
            return None
        except Exception as exc:
            log.error("extraction failed: {}", exc)
            # extract() already rolled back the temp directory.
            return None
